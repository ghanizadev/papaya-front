...
import {Background, Logo, SideBar, Header, SubHeader, Button, Container, Overlay} from './components';
import {TablesOrders, ProductInterface} from './views';
...
const Home = (props) => {
	...
	if (cookies.authorization){ //TODO pegar info do usuario no banco
	...
	const getCurrentPage = () => {
		switch(page){
			case 'orders': return <TablesOrders />
			case 'products': return <ProductInterface />
			default: return <TablesOrders />
		}
	}
		...
		<Provider>
			<Background>
				<Header>
					...
					<div>
						<span>Bem vindo {user}! <a href='#' onClick={() =>{
							history.push('/');
