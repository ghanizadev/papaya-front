  ...
  return (
    <Tabs>
      ...
      <TabPanel style={{ padding: 25 }}>
        ...
        <div style={{
          display: 'flex', flexDirection: 'row', position: 'absolute', right: 25, bottom: 25,
        }}
        >

          <Button
            title="Salvar o produto atual"
            onClick={() => {
              console.log(JSON.stringify(body));

              if (cookies.authorization) {
                save(cookies.authorization, body)
                  .then(() => {
                    window.alert('Fornecedor salvo com sucesso!');
                  })
                  .catch((error) => window.alert(`Erro ao salvar o fornecedor (${error.message})`));
              }
            }}
          >
            Salvar
